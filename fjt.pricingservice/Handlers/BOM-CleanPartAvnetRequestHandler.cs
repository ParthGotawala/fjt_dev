...
{
    public class BOM_CleanPartAvnetRequestHandler : IBOM_CleanPartAvnetRequestHandler
    {
        IModel channel = null;
        string bomServiceQueue = ConfigurationManager.AppSettings["BOMCleanAVQueue"].ToString();
        ...
        private void ProcessMessages(BasicDeliverEventArgs Model, IModel channel, int processCount)
        {
            try
            {
                ...
                IAvnetPartHandler _IPricingRequest = UnityConfig.Container.Resolve<IAvnetPartHandler>();
