...
{
    public class BOM_CleanPartTTIRequestHandler: IBOM_CleanPartTTIRequestHandler
    {
        IModel channel = null;
        string bomServiceQueue = ConfigurationManager.AppSettings["BOMCleanTTIQueue"].ToString();
        ...
        private void ProcessMessages(BasicDeliverEventArgs Model, IModel channel, int processCount)
        {
            try
            {
                ...
                ITTIPartUpdateHandler _IPricingRequest = UnityConfig.Container.Resolve<ITTIPartUpdateHandler>();
