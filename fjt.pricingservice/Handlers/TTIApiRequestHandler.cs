...
{
    public class TTIApiRequestHandler : ITTIApiRequestHandler
    {
        IModel channel = null;
        ...
        ushort TTIConsumerPrefetchCount = ushort.Parse(ConfigurationManager.AppSettings["TTIConsumerPrefetchCount"].ToString());
        /// <summary>
        ...
        public void Process()
        {
            try
            {
                ...
                {
                    channel.QueueDeclare(ttiKeyServiceQueue, false, false, false, null);
                }

                channel = ConstantHelper.connection.CreateModel();
                channel.BasicQos(0, TTIConsumerPrefetchCount, false);
                ...
                consumer.ConsumerTag = channel.BasicConsume(ttiKeyServiceQueue, false, consumer);
            }
            catch (Exception ex)
            {
