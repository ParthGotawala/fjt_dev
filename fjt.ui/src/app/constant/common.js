...
function removeOidcValuesFromLocalStorage() {
  const oidcArr = []; // Array to hold the keys
  // Iterate over localStorage and insert the keys that meet the condition into arr
  for (let i = 0; i < localStorage.length; i++) {
    if (localStorage.key(i).substring(0, 4) === 'oidc') {
      oidcArr.push(localStorage.key(i));
    }
  }
  // remove oidc library values from localStorage.
  _.forEach(oidcArr, (item) => localStorage.removeItem(item));
}
...
function setFocusByName(name, isSelectAll) {
  setTimeout(() => {
    ...
    if (element) {
      ...
      if (!isSelectAll && element.prop('nodeName') !== 'INPUT') {
        element.focus();
      } else {
        element.select().focus();
      }
...
/**
Replace  encryptAESForIDS with encryptAES.
 */

//function encryptAESForIDS(text) {
//  var iv = CryptoJS.enc.Utf8.parse('fl!23net@%$$2!@#');
//  return CryptoJS.AES.encrypt(text, iv, {
//    keySize: 128 / 8,
//    iv: iv,
...
// sample entries will be like follow
// C1,C2,C3 (OR) C1 C2 C3 (OR) C1-C3
function getDesignatorFromLineItem(refDesig, oddelyAllowedRefDes) {
  var desigArr = [];
  var refDesigArr = [];
  var DifferencedesArry = [];
  var IntersectiondesArry = [];
  if (refDesig) {
    ...
    DifferencedesArry = _.difference(refDesigArr, oddelyAllowedRefDes);
    IntersectiondesArry = _.intersection(refDesigArr, oddelyAllowedRefDes);
    ...
    IntersectiondesArry.forEach((item) => {
      desigArr.push(item);
    });

    DifferencedesArry.forEach((item) => {
      var itemIndex = item.indexOf('-');
      ...
      if (itemIndex > 0 && item.length !== (itemIndex + 1)) {
        ...
        if (firstNumberMatch && lastNumberMatch) {
          ...
          const prefixFirstNumber = stringReplaceFromIndex(itemArr[0], firstNumberMatch.index, '');
          const prefixLastNumber = stringReplaceFromIndex(itemArr[1], lastNumberMatch.index, '');

          const min = Math.min(firstNumber, lastNumber);
          const max = Math.max(firstNumber, lastNumber);

          if ((prefixLastNumber && prefixFirstNumber === prefixLastNumber) || (!prefixLastNumber && prefixFirstNumber)) {
            for (let i = min; i <= max; i++) {
              desigArr.push(prefixFirstNumber + i);
            }
          } else {
            const isDecimal = item.includes('.');
            if (isDecimal) {
              desigArr = [];
            }
          }
...
function roundUpNum(number, Precision) {
  return parseFloat((number + Number.EPSILON).toFixed(Precision));
}
...
function validateTwoDecimalOnly(t) {
  //var t = e.value;
  //e.value = (t.indexOf(".") >= 0) ? (t.substr(0, t.indexOf(".")) + t.substr(t.indexOf("."), 3)) : t;
  /* Note: commented because here e.value not give decimal point like "1." it gives as "1". actual function code is oky */
  t = (t.indexOf(".") >= 0) ? (t.substr(0, t.indexOf(".")) + t.substr(t.indexOf("."), 3)) : t;
};

// get Tool tip from list of value array
function getFilterTooltipWithoutModel(selectedModel, valueFieldName) {
  var maxTooltipLimit = 10;
  var isTooltipGreatrtthenLimit = false;
  var moreTooltipText = '<br />more...';
  var toolTipText = '';
...
function restrictCommaandSpace(evt) {
  var charCode = (evt.which) ? evt.which : event.keyCode;
