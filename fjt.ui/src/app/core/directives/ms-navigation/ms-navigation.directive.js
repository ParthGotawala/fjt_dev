(function () {
  ...
  function msNavigationServiceProvider() {
    ...
    function _byWeight(x, y) {
      ...
      return parseFloat(x.weight) - parseFloat(y.weight);
    }
    ...
    this.$get = function () {
      ...
      //add crumbs for sitemap
      function addCrumb(item, scope) {
  ...
  /** @ngInject */
  function MsNavigationController($scope, msNavigationService, BaseService, DialogFactory, TRANSACTION, TASK, $timeout, $rootScope, socketConnectionService, CORE) {
    var vm = this;
    ...
    function init() {
      ...
      // check transfer material page rights given or not
      //vm.transferPageRight = () => {
      //  vm.istransferPageRight = false;
      //  if (_.find(vm.navigation, (item) => { return item.state === TASK.TASK_MANAGE_STATE })) {
      //    vm.istransferPageRight = true;
      //  } else {
      //    let pageRights = _.find(vm.navigation, (item) => { return item.title === TRANSACTION.TRANSACTION_MATERIAL_MANAGEMENT });
      //    if (pageRights && (_.find(pageRights.children, (item) => { return item.state === TRANSACTION.TRANSACTION_RECEIVINGMATERIAL_LIST_STATE }))) {
      //      vm.istransferPageRight = true;
      //    }
      //  }
      //  return vm.istransferPageRight;
      //}
    }
  ...
  /** @ngInject */
  function MsNavigationNodeController($scope, $element, $rootScope, $animate, $state, msNavigationService, BaseService) {
    var vm = this;
    ...
    vm.username = BaseService.loginUser.username;
  ...
  /** @ngInject */
  function MsNavigationHorizontalNodeController($scope, $element, $rootScope, $state, msNavigationService, BaseService, DialogFactory, TRANSACTION, $timeout, socketConnectionService, CORE) {
    var vm = this;
    ...
    // Don't Remove this code - Vaibhav Shah
    vm.SetMenuPosition = (event) => {
      var currentElement = this.element;
      var offset = this.element.offset();
      var eleWidth = currentElement.width();
      let leftPos = offset.left + eleWidth;
      let topPos = offset.top;
      if (currentElement[0].children.length > 0) {
        _.each(currentElement[0].children, (ele) => {
          if (ele.tagName == "UL") {
            ele.style.left = leftPos + "px";
            ele.style.top = topPos + "px";
            let bodyHeight = document.body.offsetHeight;
            if (ele.clientHeight >= bodyHeight - topPos) {
              //ele.style.top = (ele.clientHeight - screenHeight - topPos - 50) + "px";
              //console.log(topPos, ele.clientHeight, bodyHeight);
              ele.style.top = (topPos - (topPos + ele.clientHeight - bodyHeight) - 10) + "px";
            }
          }
        });
      }
      //$(this).children("ul").css("left", abc.left + 100);
    }
    // Don't Remove this code - Vaibhav Shah
    ...
    function init() {
      ...
      if (vm.node.state === $state.current.name || $state.includes(vm.node.state)) {
        ...
        var navigationItems = msNavigationService.getNavigation();
        var childMatch = null;
        var parentNode = _.find(navigationItems, function (item) {
          if (item.children && item.children.length) {
            var childLevel = null;
            var child = _.find(item.children, function (childItem) {
              if (childItem.children && childItem.children.length) {
                childLevel = _.find(childItem.children, function (childItemlevel) {
                  return childItemlevel.state && (childItemlevel.state == $state.current.parentState || childItemlevel.state == $state.current.name);
                });

                if (childLevel) {
                  childMatch = childLevel;
                  return true;
                }
              } else {
                return childItem.state && (childItem.state == $state.current.parentState || childItem.state == $state.current.name);
              }
            });

            if (child) {
              childMatch = childLevel || child;
              return true;
            }
          } else if (item.state == $state.current.parentState)
            return true;
        ...
        if (parentNode == vm.node) {
          msNavigationService.reset();
          msNavigationService.addCrumb(parentNode);
          if (childMatch)
            msNavigationService.addCrumb(childMatch);

          $scope.$emit('msNavigation.breadcrumb');
        }
      }
      ...
      $scope.$on('msNavigation::clearActive', function () {
        var activeItem = msNavigationService.getActiveItem();
        if (!vm.hasChildren) {
          //add only root node active items into crumbs
          msNavigationService.addCrumb(activeItem.node);
          return;
        }
        ...
        let active = activePathParts;
        // Test for active path
        if (activePathParts.indexOf(vm.node._id) > -1) {
          let breadScrumbMethodTimeout = null;
          $scope.$evalAsync(function () {
            //add only node active items into crumbs
            msNavigationService.addCrumb(vm.node);
            //add active items into crumbs
            msNavigationService.addCrumb(activeItem.node);
            _.each(vm.node.children, (ele) => {
              if (activePathParts.indexOf(ele._id) > -1) {
                //add active items into crumbs
                msNavigationService.addCrumb(ele);
              }
            });
            // ------------------ Bug 40690: Breadcrumb is not loading on Page load --------------------------------
            clearTimeout(breadScrumbMethodTimeout);
            breadScrumbMethodTimeout = setTimeout(() => {
              $scope.$emit('msNavigation.breadcrumb');
            }, _configTimeout);
            // ------------------ Bug 40690: Breadcrumb is not loading on Page load --------------------------------
            vm.isActive = true;
    ...
    vm.openPopup = (functionName) => {
      BaseService[functionName].call();
    };
  };
