...
angular.module('app.core')
  ...
  .directive('datetimePicker', ['$compile', '$parse', '$document', '$timeout', '$uibPosition', 'dateFilter', 'uibDateParser', 'uiDatetimePickerConfig', '$rootScope', '$window',
    function ($compile, $parse, $document, $timeout, $uibPosition, dateFilter, uibDateParser, uiDatetimePickerConfig, $rootScope, $window) {
      return {
        restrict: 'A',
        require: 'ngModel',
        scope: {
        ...
        link: function (scope, element, attrs, ngModel) {
          ...
          //scope.allowFutureDate = scope.allowFutureDate ? scope.allowFutureDate : true;
          //scope.allowPastDate = scope.allowPastDate ? scope.allowPastDate : true;
          ...
          if (uiDatetimePickerConfig.html5Types[attrs.type]) {
            ...
            attrs.$observe('datetimePicker', (value)=> {
          ...
          //Added for popup placement issue - Vaibhav Shah - 09/12/2020
          let scrollParentEl;
          let onOpenFocus = true;
          let popupPlacement = 'auto bottom-left';

          function positionPopup() {
            if (scope.isOpen) {
              var dpElement = angular.element($popup[0]);
              var placement = attrs.popupPlacement ? attrs.popupPlacement : popupPlacement;
          ...
          scope.$watch('isOpen', (value)=> {
            if (value) {
              if (!scope.disabled) {
                if (!scope.date) {
                  scope.date = new Date(); // added to remove focus from last selected date (28-10-2021)-CC
                }
                $timeout(()=> {
                  positionPopup();
                  if (onOpenFocus) {
                  ...
                  const placement = attrs.popupPlacement ? attrs.popupPlacement : popupPlacement;
          ...
          scope.watchData = {};
          angular.forEach(['minMode', 'maxMode', 'minDate', 'maxDate', 'datepickerMode', 'initDate', 'shortcutPropagation', 'datepickerOptions'], function (key) {
            if (attrs[key]) {
          ...
          function maxValueValidator(modelValue, viewValue) {
            var value = modelValue || viewValue;
            // max value check with today date
            // max value check with min date
            if (attrs.maxValue && value) {
              ...
              let maxValueDate;
              if (scope.maxValue) {
                maxValueDate = new Date(scope.maxValue);
              }

              if (!scope.enableTime) {
                todayDate = todayDate.setHours(0, 0, 0, 0);
                viewValueDate = _.isDate(viewValueDate) ? viewValueDate.setHours(0, 0, 0, 0) : null;
                maxValueDate = _.isDate(maxValueDate) ? maxValueDate.setHours(0, 0, 0, 0) : null;
              }

              if (scope.allowFutureDate) {
                if (scope.maxValueIsTodayDate) {
                  if (viewValueDate && _.isDate(new Date(viewValueDate)) && (todayDate > viewValueDate)) {
                    return false;
                  }
                } else {
                  if (maxValueDate && viewValueDate && (_.isDate(new Date(maxValueDate)) && _.isDate(new Date(viewValueDate))) && (maxValueDate < viewValueDate)) {
                    return false;
                  }
                }
                return true;
              }
              if (scope.allowPastDate) {
                if (scope.maxValueIsTodayDate) {
                  if (viewValueDate && _.isDate(new Date(viewValueDate)) && (todayDate < viewValueDate)) {
                    return false;
                  }
          ...
          function minValueValidator(modelValue, viewValue) {
            var value = modelValue || viewValue;
            //min value check with strat date
            if (attrs.minValue && value) {
              let todayDate = new Date();
              let viewValueDate;
              if (viewValue) {
                viewValueDate = new Date(viewValue);
              ...
              let minValueDate;
              if (scope.minValue) {
                minValueDate = new Date(scope.minValue);
              }

              if (!scope.enableTime) {
                todayDate = todayDate.setHours(0, 0, 0, 0);
                viewValueDate = _.isDate(viewValueDate) ? viewValueDate.setHours(0, 0, 0, 0) : null;
                minValueDate = _.isDate(minValueDate) ? minValueDate.setHours(0, 0, 0, 0) : null;
              }

              if (scope.allowFutureDate) {
                if (scope.minValueIsTodayDate) {
                  if (viewValueDate && _.isDate(new Date(viewValueDate)) && (todayDate > viewValueDate)) {
                    return false;
                  }
                } else {
                  if (minValueDate && viewValueDate && (_.isDate(new Date(minValueDate)) && _.isDate(new Date(viewValueDate))) && (minValueDate > viewValueDate)) {
                    return false;
                  }
                }
                return true;
          ...
          function validator(modelValue, viewValue) {
            ...
            if (!value) {
              ...
              if (value.toLowerCase() == dateFormat.toLowerCase()) {
                return true;
              }
          ...
          if (!isHtml5DateInput) {
            ...
            ngModel.$validators.maxvalue = maxValueValidator;
            ngModel.$validators.minvalue = minValueValidator;
          ...
          scope.dateSelection = function (dt) {
            ...
            if (!ngModel.$viewValue && scope.date) {
              var now = new Date();
              scope.date.setHours(now.getHours());
              scope.date.setMinutes(now.getMinutes());
              scope.date.setSeconds(now.getSeconds());
              scope.date.setMilliseconds(now.getMilliseconds());
            }
          ...
          scope.$watch('isOpen', function (value) {
            ...
            if (value) {
              ...
              //scope.dropdownStyle.left = (position.left - element.prop('offsetWidth')) + 'px';
