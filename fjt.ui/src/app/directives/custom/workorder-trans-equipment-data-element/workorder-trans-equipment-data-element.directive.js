(function () {
  ...
  angular
    .module('app.core')
    .directive('workorderTransEquipmentDataElement', workorderTransEquipmentDataElement);

  /** @ngInject */
  function workorderTransEquipmentDataElement(CORE, USER, EntityFactory, $timeout, $filter, CONFIGURATION, DialogFactory, WorkorderEquipmentDataElementTransValueFactory, BaseService, WorkorderDataElementTransValueFactory, SettingsFactory, $q, DataElementFactory) {
    var directive = {
      restrict: 'E',
      replace: true,
      scope: {
      ...
      templateUrl: 'app/directives/custom/workorder-trans-equipment-data-element/workorder-trans-equipment-data-element.html',
      controller: workorderTransEquipmentDataElementCtrl,
      controllerAs: 'vm'
    };
    return directive;
    ...
    */
    ...
    function workorderTransEquipmentDataElementCtrl($scope) {
      var vm = this;
      ...
      /* open popup and delete any row of sub-form -> not save that data -> close popup
          and open popup again then need to make data clear */
      WorkorderEquipmentDataElementTransValueFactory.clearGlobalTransIDsArrayOnInit();
      ...
      */
      vm.enityElementDetails = (entityID) => {
        if ($scope.eqpId) {
          WorkorderDataElementTransValueFactory.retrieveWorkorderOperationEquipmentDataElementList().query({
            id: entityID, woOPID: vm.woOPID, eqpID: $scope.eqpId, woID: vm.woID
          }).$promise.then((res) => {
      ...
      function retriveConfigureFileTypeList() {
        ...
        vm.cgBusyLoading = SettingsFactory.retriveConfigureFileType().query().$promise.then((response) => {
          ...
          if (response && response.data && response.data.length > 0) {
            ...
            vm.fileExtensionList = vm.FileTypeList.join(', ');
        ...
        if (data) {
          ...
          if (vm.entity === CORE.Entity.Equipment) {
            data = _.filter(data, (o) =>
              ((o.dataelement_use_at !== CORE.SHOW_ELEMENT_OPTION[0])
                && (o.workorderOperationEquipmentDataelement.length > 0 && _.find(o.workorderOperationEquipmentDataelement, (e) => e.woOPID === Number(vm.woOPID))
                ))
            );
      ...
      const getWorkorderTransEquipmentDataElementListValues = () => {
        WorkorderEquipmentDataElementTransValueFactory.getDataElementEquipmentTransactionValues().query({
          woTransID: vm.woTransId, woOPID: vm.woOPID, entityID: vm.entityId, eqpID: $scope.eqpId
        }).$promise.then((res) => {
          if (res.data) {
          ...
          .finally(() => {
            vm.getWorkorder_Trans_Equipment_SubformDataDetail();
          });
      };


      vm.getWorkorder_Trans_Equipment_SubformDataDetail = () => {
        const refWoTransEqpSubFormDataIDs = _.filter(vm.elementTransactionList, (obj) => (obj.refWoTransEqpSubFormDataID))
          .map((obj) => obj.refWoTransEqpSubFormDataID);
      ...
      */
      ...
      const downloadElementDocument = (itemElement) => WorkorderEquipmentDataElementTransValueFactory.downloadDocument(itemElement.woTransEqpDataElementID).then((response) => {
      ...
      vm.DeleteSubFormRecord = (itemElement, subElement) => {
        ...
        const subFormTransIDList = _.compact(_.uniq(_.map(subElement, 'woTransEqpSubFormDataID')));
        ...
        _.each(itemElement.subFormControls, (item) => {
          ...
          _.remove(item, (obj) => _.includes(vm.deletedsubFormTransIDs, obj.woTransEqpSubFormDataID));
          // for all rownumbers
        ...
        WorkorderEquipmentDataElementTransValueFactory.assignDeletedsubFormTransID(vm.deletedsubFormTransIDs);
        manuallyEnableSaveButton();
      };
      ...
      vm.previewDocument = (ev, itemElementData) => {
        ...
        if (itemElementData.fileDetail.mimetype.indexOf('/pdf') !== -1) {
          // if pdf file type then get its details to pass in preview popup other all type we have path
