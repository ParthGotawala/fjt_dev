...
/***************************************** CUSTOM CHANGES *********************************************/
// LINE 442 : scope.defineColumns = function () {
// LINE 551 : this.setColumnSettingArray = function (column) {
// LINE 589 : if (attrs.dynamiccontext)
// LINE 671 : scope.$watchCollection('dynamiccontext'
// LINE 696 : Set dynamic Context menu after min row space 
/*******************************************************************************************************/
        ...
        return {
            ...
            compile: function (tElement, tAttrs) {
                ...
                return function (scope, element, attrs, controllerInstance) {
                    // custom change to increase performance
                    scope.defineColumns = function () {
                        scope.columns = [];
                        var mappingData = _.filter(scope.mappingData, (item) => { return !item.hidden; });
                        for (var i = 0, len = mappingData.length; i < len; i++) {
                            var item = mappingData[i];

                            attrs.data = item.field;
                            ...
                            if (item.validator)
                                attrs.validator = item.validator;
                            ...
                            angular.extend(scope.columns[i], column);
                        }

                        controllerInstance.setColumnSettingArray(scope.columns);
                    }

                    scope.defineColumns();

                    scope.destroyColumns = function () {
                        controllerInstance.removeColumnSettingArray(scope.columns);
                    }
    ...
    function hotTable(settingFactory, autoCompleteFactory, $rootScope, $parse) {
        return {
            ...
            controller: ['$scope', function ($scope) {
                ...
                // custome change
                this.setColumnSettingArray = function (column) {
                    if (!$scope.htSettings) {
                        $scope.htSettings = {};
                    }
                    if (!$scope.htSettings.columns) {
                    ...
                    angular.forEach(column, function (col) { $scope.htSettings.columns.push(col); });

                    settingFactory.updateHandsontableSettings($scope.hotInstance, $scope.htSettings);
                };
                this.removeColumnSettingArray = function (column) {
                    angular.forEach(column, function (col) {
                        if ($scope.htSettings.columns.indexOf(col) > -1) {
                            $scope.htSettings.columns.splice($scope.htSettings.columns.indexOf(col), 1);
                        }
                    });
                    settingFactory.updateHandsontableSettings($scope.hotInstance, $scope.htSettings);
                };
            ...
            compile: function (tElement, tAttrs) {
                ...
                return function (scope, element, attrs) {
                    ...
                    if (attrs.width == undefined && attrs.ispopup != undefined && attrs.ispopup == "true") {
                        scope.settings.width = element.parent()[0].offsetWidth - 40;
                    }

                    // custom change
                    if (attrs.dynamiccontext)
                        scope.dynamiccontext = $parse(attrs.dynamiccontext)(scope.$parent);
                    ...
                    scope.$watchCollection('dynamiccontext', function (newvalue, oldvalue) {
                        settingFactory.updateHandsontableSettings(scope.hotInstance, scope.dynamiccontext);
                        // alert('context call');
                    });
                    ...
                    scope.$watchCollection('datarows', function (newValue, oldValue) {
                        if (oldValue && oldValue.length === scope.htSettings.minSpareRows && newValue.length !== scope.htSettings.minSpareRows) {
                            scope.htSettings.data = scope.datarows;
                            settingFactory.updateHandsontableSettings(scope.hotInstance, scope.htSettings);
                            // Set dynamic Context menu after min row space 
                            settingFactory.updateHandsontableSettings(scope.hotInstance, scope.dynamiccontext);
                        }
                    });
                };
