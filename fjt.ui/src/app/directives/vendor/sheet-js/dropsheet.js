...
var DropSheet = function DropSheet(opts) {
    ...
    function sheetjsw(data, cb, readtype, file) {
        pending = true;
        opts.on.workstart();
        ...
        var worker = new Worker(window.location.origin + '/config/sheet-js/sheetjsw.js');
        worker.onmessage = function (e) {
            switch (e.data.t) {
                case 'ready': break;
                case 'e': pending = false; return opts.errors.badfile(); break;
                case 'xlsx':
                    pending = false;
                    opts.on.workend();
                    if (e.data.d)
                        cb(JSON.parse(e.data.d), null, file);
                    break;
            }
        };
    ...
    function to_json(workbook) {
        ...
        workbook.SheetNames.forEach(function (sheetName) {
          var roa = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { header: 1, raw: true });
    ...
    function process_wb(wb, sheetidx, file) {
        last_wb = wb;
        opts.on.wb(wb, sheetidx);
        var sheet = wb.SheetNames[sheetidx || 0];
        var json = to_json(wb)[sheet];
        opts.on.sheet(json, wb.SheetNames, choose_sheet, file);
    }
    ...
    function handleDrop(e) {
        ...
        if (opts.multiple === false && files.length > 1) {
            if (opts.errors.multiplefile)
                opts.errors.multiplefile(files.length);
            return;
        }
        ...
        for (i = 0, f = files[i]; i != files.length; ++i) {
            ...
            reader.onload = function (e) {
              ...
              var wb, arr;
              var readtype = { type: rABS ? 'binary' : 'base64', dateNF: 'mm/dd/yyyy;@', cellDates: true };
                if (!rABS) {
                    arr = fixdata(data);
                    data = btoa(arr);
                }
                function doit() {
                  try {
                        //if (useworker) { sheetjsw(data, process_wb, readtype, f); return; }
                        wb = XLSX.read(data, readtype);
                        process_wb(wb, null, f);
        ...
        for (i = 0, f = files[i]; i != files.length; ++i) {
            ...
            reader.onload = function (e) {
              ...
              var readtype = { type: rABS ? 'binary' : 'base64', dateNF: 'mm/dd/yyyy;@', cellDates: true };
                ...
                if (e.target.result.length > 2e6) opts.errors.large(e.target.result.length, function (e) { if (e) doit(); });
                else { doit(); }
            };
            if (rABS) reader.readAsBinaryString(f);
            else reader.readAsArrayBuffer(f);
