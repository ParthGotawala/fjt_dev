...
function sheet_to_json(sheet, opts) {
	...
	for (R = r.s.r + offset; R <= r.e.r; ++R) {
		...
		if(!dense || sheet[R]) for (C = r.s.c; C <= r.e.c; ++C) {
          ...
          row[hdr[C]] = raw ? ((val && val.t != 'd') ? v : format_cell(val, v, o)) : format_cell(val, v, o); // Change this condition for number format issue like 2253+E11 show on number format on 15/04/2020 By JK
