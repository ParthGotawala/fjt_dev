...
//******************************************** Custom changes *************************************************************************//
//
// line 1667: source.text().replace(/\ufeff/g, '').length <= max;
// ling 1692: !source.text().replace(/\ufeff/g, '').length || source.text().replace(/\ufeff/g, '').length >= min;
//
//*************************************************************************************************************************************//
...
angular.module('textAngular.factories', [])
  .factory('taBrowserTag', [function () {
    ...
    var taFixChrome = function (html, keepStyles) {
      ...
      var match, styleVal, appleSpaceVal, newTag, finalHtml = '',
        lastIndex = 0;
    ...
    function transformLegacyStyles(html) {
      ...
      var match, subMatch, styleVal, newTag, lastNewTag = '',
        newHtml, finalHtml = '',
        lastIndex = 0;
...
angular.module('textAngular.DOM', ['textAngular.factories'])
  .factory('taExecCommand', ['taSelection', 'taBrowserTag', '$document', function (taSelection, taBrowserTag, $document) {
    ...
    return function (taDefaultWrap, topNode) {
      ...
      return function (command, showUI, options, defaultTagAttributes, textEditorSerial) {
        ...
        if (command.toLowerCase() === 'insertorderedlist' || command.toLowerCase() === 'insertunorderedlist') {
          ...
          var tagBegin = '<a href="' + options + '" target="' +
            (defaultTagAttributes.a.target ? defaultTagAttributes.a.target : '_blank') +
            '">',
            tagEnd = '</a>&nbsp;&nbsp;',
          ...
          // [Dharam: 04/02/2020] Added parameter 'textEditorSerial' to get text editor element ID
          taSelection.insertHtml(options, topNode, textEditorSerial);
          return;
        }
        try {
    ...
    function ($document, taDOM, $log) {
      ...
      var api = {
        ...
        // [Dharam: 04/02/2020] Added parameter 'textEditorSerial' to get text editor element ID
        insertHtml: function (html, topNode, textEditorSerial) {
          var parent, secondParent, _childI, nodes, i, lastNode, _tempFrag;
          var element = angular.element("<div>" + html + "</div>");
          var range = rangy.getSelection().getRangeAt(0);
          //[S] [Dharam: 03/31/2020] Resolved issue of upload image/video/link when remove all content by ctrl + a + remove by backspace or delete
          var isHtmlInTextAngular = $(range.commonAncestorContainer).closest("text-angular");
          if (textEditorSerial && (!isHtmlInTextAngular || !isHtmlInTextAngular[0])) {
            var parentRange = rangy.createRange();
            var el = $('#taTextElement' + textEditorSerial + ' p').last()[0];
            parentRange.selectNodeContents(el);
            parentRange.collapse(true);
            rangy.getSelection().setSingleRange(parentRange);
            range = rangy.getSelection().getRangeAt(0);
          } else if ($(range.commonAncestorContainer).attr('ta-bind') == 'ta-bind') {
            var parentRange = rangy.createRange();
            var el = $(range.commonAncestorContainer).find('p')[0];
            parentRange.selectNodeContents(el);
            parentRange.collapse(true);
            rangy.getSelection().setSingleRange(parentRange);
            range = rangy.getSelection().getRangeAt(0);
          }
          //[E] [Dharam: 03/31/2020] Resolved issue of upload image/video/link when remove all content by ctrl + a + remove by backspace or delete
    ...
    return {
      ...
      link: function (scope, elem, attrs, ctrl) {
        ...
        ctrl.$validators.taMaxText = function (viewValue) {
          ...
          return source.text().replace(/\ufeff/g, '').length <= max;
        };
      }
    };
    ...
    return {
      ...
      link: function (scope, elem, attrs, ctrl) {
        ...
        ctrl.$validators.taMinText = function (viewValue) {
          var source = angular.element('<div/>');
          source.html(viewValue);
          return !source.text().replace(/\ufeff/g, '').length || source.text().replace(/\ufeff/g, '').length >= min;
        };
      }
    };
  });
angular.module('textAngular.taBind', ['textAngular.factories', 'textAngular.DOM'])
  ...
  .directive('taBind', [
    ...
    '_taBlankTest', '$parse', 'taDOM', 'textAngularManager', 'CORE', 'GenericFileFactory',
    function (
      taSanitize, $timeout, $document, taFixChrome, taBrowserTag,
      taSelection, taSelectableElements, taApplyCustomRenderers, taOptions,
      _taBlankTest, $parse, taDOM, textAngularManager, CORE, GenericFileFactory) {
            ...
            if (!_isContentEditable) {
              ...
              var processpaste = function (text) {
                ...
                if (text && text.trim().length) {
                  ...
                  if (_pasteHandler) text = _pasteHandler(scope, {
                    $html: text
                  }) || text;
              ...
              element.on('paste', scope.events.paste = function (e, eventData) {
                ...
                if (clipboardData && clipboardData.getData && clipboardData.types.length > 0) { // Webkit - get data from clipboard, put into editdiv, cleanup, then cancel event
                  ...
                  if (clipboardData.files.length > 0) {
                    ...
                    scope.fileUpload(clipboardData, e, 'paste');
                    //var reader = new FileReader();
                    //reader.readAsDataURL(clipboardData.files[0]);
                    //reader.onload = function () {
                    //    pastedContent = '<img src="' + reader.result + '" />';
                    //    processpaste(pastedContent);
                    //};
                  }
                  processpaste(pastedContent);
                ...
                if (!_isReadonly && !BLOCKED_KEYS.test(event.keyCode)) {
                  ...
                  if (event.keyCode === _ENTER_KEYCODE && (event.ctrlKey || event.metaKey || event.altKey)) {
                    ...
                    if (_defaultVal !== '' && _defaultVal !== '<BR><BR>' && event.keyCode === _ENTER_KEYCODE && !event.ctrlKey && !event.metaKey && !event.altKey) {
                      var selection = taSelection.getSelectionElement();
                      while ((selection && !selection.nodeName.match(VALIDELEMENTS)) && selection !== element[0]) {
                      ...
                      if (!event.shiftKey) {
                        ...
                        if (selection && selection.tagName.toLowerCase() !==
                          attrs.taDefaultWrap &&
...
textAngular.directive("textAngular", [
  ...
  'textAngularManager', '$document', '$animate', '$log', '$q', '$parse', 'GenericFileFactory',
  'BaseService', 'CORE', 'Upload', 'DialogFactory',
  function ($compile, $timeout, taOptions, taSelection, taExecCommand,
    textAngularManager, $document, $animate, $log, $q, $parse, GenericFileFactory, BaseService, CORE, Upload, DialogFactory) {
    return {
      require: '?ngModel',
      scope: {
        cgBusyLoading: '=?'
      ...
      link: function (scope, element, attrs, ngModel) {
        ...
        angular.extend(scope, angular.copy(taOptions), {
          ...
          wrapSelection: function (command, opt, isSelectableElementTool) {
            ...
            if (command.toLowerCase() === "undo") {
              ...
              _taExecCommand(command, false, opt, scope.defaultTagAttributes, _serial);
        ...
        //[Dharam] Support video types
        scope.allowVideoTypes = ['video/mp4', 'video/webm', 'video/ogg'];

        scope.allowVideoTypeExtension = _.map(_.filter(CORE.FileTypeList, (obj) => {
          return scope.allowVideoTypes.indexOf(obj.mimetype) != -1
        }), 'extension').join(', ');
        ...
        scope.displayElements = {
          ...
          forminput: angular.element("<input spellcheck='true' type='hidden' tabindex='-1' style='display: none;'>"),
          html: angular.element("<textarea spellcheck='true'></textarea>"),
          text: angular.element("<div no-translate-class></div>"),
          /* no-translate-class dir- as text-editor not reqired to translate in selected other language */
        ...
        scope.reflowPopover = function (_el) {
          ...
          if (spaceAboveImage < 51) {
            //[Dharam: 03/31/2020] Resolved issue of display pop over on link/image/video
            //scope.displayElements.popover.css('top', _el[0].offsetTop + _el[0].offsetHeight + scope.displayElements.scrollWindow[0].scrollTop + 'px');
            //console.log('bottom', _el[0].offsetTop + _el[0].offsetHeight + scope.displayElements.scrollWindow[0].scrollTop);
            //scope.displayElements.popover[0].style.setProperty('top', _el[0].offsetTop + _el[0].offsetHeight + scope.displayElements.scrollWindow[0].scrollTop + 'px', 'important');
            scope.displayElements.popover[0].style.setProperty('top', (_el[0].offsetTop + _el[0].offsetHeight) + 'px', 'important');
            scope.displayElements.popover.removeClass('top').addClass('bottom');
          } else {
            //[Dharam: 03/31/2020] Resolved issue of display pop over on link/image/video
            //scope.displayElements.popover.css('top', _el[0].offsetTop - 54 + scope.displayElements.scrollWindow[0].scrollTop + 'px');
            //console.log('top', _el[0].offsetTop - 54 + scope.displayElements.scrollWindow[0].scrollTop);
            //scope.displayElements.popover[0].style.setProperty('top', _el[0].offsetTop - 54 + scope.displayElements.scrollWindow[0].scrollTop + 'px', 'important');
            scope.displayElements.popover[0].style.setProperty('top', (_el[0].offsetTop - 54) + 'px', 'important');
          ...
          scope.displayElements.popover[0].style.setProperty('left', _rleft + 'px', 'important');
          ...
          ngModel.$render = function () {
            if (_firstRun) {
              ...
              if ((_initialValue === undefined || _initialValue === null) && (_originalContents && _originalContents !== '')) {
                ...
                /** Dharam: 12/14/2019 Below code is commented to resolve issue of form dirty when editor is initialize. Below code bind value of character of enter key in model.*/
        ...
        scope.$watch('html', function (newValue, oldValue) {
          if (newValue !== oldValue) {
            ...
            //setTimeout(function () {
            //  $("#taTextElement" + _serial + ' img').on('load', function () {
            //    //console.log(this);
        ...
        ****************/
        ...
        scope.$watch('loaderVisible', function (newValue) {
          if (newValue) {
            ...
            scope.cgBusyLoading = scope.timeoutWatch;
          } else {
        ...
        if (!r.support) {
          ...
          r.on('fileSuccess', function (file, message) {
            let filePath, options, fileType;
            //let filetypes = ['video/mp4', 'video/webm', 'video/ogg'];
            //once saved ,get image path detail.
            let response = JSON.parse(message);
            if (response && response.data && file && file.file) {
              fileType = file.file.type;
              filePath = response.data;
              filePath = CORE.WEB_URL + filePath.substring(2, filePath.length);
              let isVideo = _.find(scope.allowVideoTypes, function (item) {
                return item == fileType
              });
              if (isVideo) {
                options = '<div><b style="font-size: 16px;text-align: left;cursor: pointer;"><a href="' + filePath + '" target="_blank">' + file.file.name + '</a></b><br/></div>';
              } else {
                options = '<div><br/><img src="' + filePath + '"><br/></div><br/>';
                //options = '<br/><img src="' + filePath + '"><br/>';
              }
              //$timeout(() => {
              taSelection.insertHtml(options, null);
              //});
            }
          });
          ...
          r.on('catchAll', function (event, file, message) {
            if ((event === 'fileAdded' || event === 'filesAdded') && file.length === 0) {
              // Hide pause/resume when the upload has completed
              scope.loaderVisible = false;
              BaseService.currentPageFlagForm = [];
              $('.resumable-list').empty();
              $('.resumable-progress').hide();
              const messageContent = angular.copy(CORE.MESSAGE_CONSTANT.DYNAMIC_MESSAGE.GLOBAL.DOCUMENT_ALREADY_ADDED);
              messageContent.message = stringFormat(messageContent.message, "File(s)");
              if (messageContent) {
                var model = {
        ...
        //Capture image from toolbar option
        scope.cameraFileUpload = (file) => {
          try {
            scope.loaderVisible = true;
            BaseService.currentPageFlagForm = [true];
            let imageResult = Upload.dataUrltoBlob(file.finaldataURL, file.name);
            let documentDetail = {
              'gencFileOwnerType': 'textAngular',
              'originalname': imageResult.name,
            ...
            //r.files.push(files);
            r.addFile(imageResult);

            ////save image in API folder.
            ////[Dharam: 03/31/2020] Implement loader for upload file
            //scope.cgBusyLoading = GenericFileFactory.uploadGenericFiles(documentDetail, imageResult).then((response) => {
            //  let filePath, options;

            //  //once saved ,get/set image path detail.
            ...
            //    options = '&nbsp;<br><img src="' + filePath + '"></br>&nbsp;';
            //    // Added by Leena for POC - 07/02/2020
            //    //options = '&nbsp;<br><img src="' + filePath + '" contenteditable="false" datasrc="' + response.data.data.substring(2, response.data.data.length) + '"></br>&nbsp;';
            //    //let selection = taSelection.getSelectionElement();
            //    //taSelection.setSelectionToElementEnd(selection);
            //    // Added by Leena for POC - 07/02/2020
            //    //$(taSelection.getSelectionElement()).attr('ng-click', 'OnClickImage()')
            //    taSelection.insertHtml(options);
            //    $timeout(function () {
            //      //if (element[0].getElementsByClassName("notranslate") && element[0].getElementsByClassName("notranslate")[0]) {
            //      //    ngModel.$setViewValue(element[0].getElementsByClassName("notranslate")[0].innerHTML);
            //      //}
            //      //ngModel.$setViewValue(element[0].innerHTML);
            //      ngModel.$setViewValue('&nbsp;<br>&nbsp;');
            //    }, 0);
            //  }
            //}).catch((error) => {
        ...
        //Upload video in editor using video upload option from toolbar
        scope.videoFileUpload = (files) => {
          try {
            scope.loaderVisible = true;
            BaseService.currentPageFlagForm = [true];
            ...
            //  //save image in API folder.
            //  //[Dharam: 03/31/2020] Implement loader for upload file
            //  scope.cgBusyLoading = GenericFileFactory.uploadGenericFiles(documentDetail, file).then((response) => {
            //    let filePath, options, fileType;
            //    let filetypes = ['video/mp4', 'video/webm', 'video/ogg'];
            //    //once saved ,get image path detail.
            //    if (response && response.data && response.data.data) {
            ...
            //      filePath = response.data.data;
            //      filePath = CORE.WEB_URL + filePath.substring(2, filePath.length);
            //      //options = '<div><br/><div></div><video width="300" height="300" controls>' +
            //      //  '<source src="' + filePath + '" type="' + fileType + '">' +
            //      //  '</video><div></div><br/></div>';
            //      options = '<div><b style="font-size: 16px;text-align: left;cursor: pointer;"><a href="' + filePath + '" target="_blank">' + file.name + '</a></b><br/></div>';
            //      taSelection.insertHtml(options, null);
            //    }
            //  }).catch((error) => {
            //    return BaseService.getErrorLog(error);
        ...
        //Upload file whil drag and drop content to text editor
        scope.fileUpload = (dataTransfer, dropEvent, event) => {

          var messageContent;
          _.each(dataTransfer.files, (objFile, index) => {
            if (!messageContent) {
              if (!(objFile.type.contains('image') || objFile.type.contains('video'))) {
                messageContent = angular.copy(CORE.MESSAGE_CONSTANT.DYNAMIC_MESSAGE.GLOBAL.TEXT_EDITOR_ACCEPT_VIDEO_IMAGE);
              } else if (objFile.size > parseInt(Documentsize)) {
              ...
              } else if (objFile.type.contains('video') && scope.allowVideoTypes.indexOf(objFile.type) == -1) {
                messageContent = angular.copy(CORE.MESSAGE_CONSTANT.DYNAMIC_MESSAGE.GLOBAL.DOCUMENT_VIDEO_ALLOW_FORMAT_MSG);
                messageContent.message = stringFormat(messageContent.message, scope.allowVideoTypeExtension);
              }
            }
          });
          ...
          try {
              ...
              if (file.type == 'image/tiff') {
                ...
                reader.onload = function () {
                  ...
                  try {
                    ...
                    let finaldataURL = canvas.toDataURL();
                    let blob = Upload.dataUrltoBlob(finaldataURL, file.name);
                    let convertedFile = new File([blob], file.name, {
                      type: file.type,
            ...
            //let fileDetails = dataTransfer.files;
            //_.each(dataTransfer.files, (file) => {
            //  var documentDetail = {
            //    'gencFileOwnerType': 'textAngular',
            //    'originalname': file.name,
            //    'fileSize': file.size
            ...
            //    if (response && response.data && response.data.data) {
            //      fileType = file.type;
            //      let isVideo = _.find(scope.allowVideoTypes, function (item) { return item == fileType });
            //      if (isVideo) {
            //        filePath = response.data.data;
            //        filePath = CORE.WEB_URL + filePath.substring(2, filePath.length);
            //        //options = '<div><br/><div></div><video width="300" height="300" controls>' +
            ...
            //        options = '<br/><img src="' + filePath + '"><br/>';
            //        // Added by Leena for POC - 07/02/2020
            //        //options = '<br/><img src="' + filePath + '" attr.data-src="' + response.data.data.substring(2, response.data.data.length) + '"><br/>';
            //      }
            //      //show image in text angular area.
            //      //if (dropEvent && dropEvent.target) {
            //      //    taSelection.setSelectionToElementEnd(dropEvent.target);
            //      //}
            //      if (event == 'paste') {
            //        taSelection.insertHtml('<p>&nbsp;<p>', null);
            //      }
            //      taSelection.insertHtml(options, null);
        ...
        scope.$on('ta-drop-event', function (event, element, dropEvent, dataTransfer) {
          if (dataTransfer && dataTransfer.files && dataTransfer.files.length > 0) {
            scope.displayElements.text[0].focus();
            scope.fileUpload(dataTransfer, dropEvent, 'drop');
            //
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        var setupToolElement = function (toolDefinition, toolScope) {
          ...
          if (!showEditorSettings && toolScope.name != 'toolsSetting') {
            toolElement[0].style.display = "none";
          }
          //if (showEditorSettings && toolScope.name != 'toolsSetting') {
          //    toolElement[0].style.display = "none";
          //}
          //else {
          //    setTimeout(() => {
          //        let parentElement = toolElement.parent();
          //        parentElement.insertBefore("<span class='flex'></span>", toolElement);
          //    },1000);
          //}
        ...
        var defaultChildScope = {
          ...
          isDisabled: function () {
            ...
            if ((this.name === 'html' || this.name === 'toolsSetting' || this.name === 'toolbarFullscreen') && scope._parent.startAction) {
              return false;
            }
            ...
            return ( // this bracket is important as without it it just returns the first bracket and ignores the rest
              ...
              ((this.name !== 'html' || this.name === 'toolsSetting' || this.name === 'toolbarFullscreen') && this.$editor().showHtml) ||
