    ...
    function (selectedItemClass) {
        return {
            link: function (scope, element/*, attrs*/) {
                element.on('click', function (e, checkAll) {
                    ...
                    //if (checkAll && e.target.className.indexOf("ui-sortable-selected") == -1) {
                    //}
                });
    ...
    function (selectedItemClass) {
        ...
        return {
            ...
            helper: function (e, item) {
                ...
                var displayNodeCount;
                if (selectionCount == 0) {
                    displayNodeCount = 1;
                }
                else {
                    displayNodeCount = selectionCount;
                }
                ...
                if (uiSelectionCount && selectionCount > 1) {
                    helper.addClass('ui-selection-count')
                        .attr('data-ui-selection-count', selectionCount);
                }
                if (displayNodeCount > 1) {
                    helper.addClass("cursor-grab");
                    helper.attr("id", "newClass");
                    helper.css({ "z-index": "100000 !important", "position": "relative", "padding-left": "27px" }) // so it appears above ext-wide selection bar
                           .append(displayNodeCount + " - Item(s) selected.");
                    ...
                    //helper.attr("id", "newClass");
                    helper.css({ "z-index": "100000 !important", "position": "relative", "padding-left": "27px" }) // so it appears above ext-wide selection bar
                           .append(displayNodeCount + " - Item selected.");
                    //return helper.append(elements); // Commented due to long text half selection displayed for element block css properties
                    return helper.append();
                }
            },
            start: function (e, ui) {
