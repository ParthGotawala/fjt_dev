(function () {
  ...
  /** @ngInject */
  function CountryController($mdDialog, $scope, $q, $timeout, CORE, USER, CountryFactory, DialogFactory, BaseService) {
    const vm = this;
    vm.isUpdatable = true;
    vm.isViewAlias = true;
    ...
    const setDataAfterGetAPICall = (country, isGetDataDown) => {
      if (country && country.data && country.data.country) {
        if (!isGetDataDown) {
          vm.sourceData = country.data.country;
          vm.currentdata = vm.sourceData.length;
        }
        else if (country.data.country.length > 0) {
          vm.sourceData = vm.gridOptions.data = vm.gridOptions.data.concat(country.data.country);
        ...
        vm.totalSourceDataCount = country.data.Count;
    ...
    //get country data for grid bind
    vm.loadData = () => {
      BaseService.setPageSizeOfGrid(vm.pagingInfo, vm.gridOptions);
      vm.cgBusyLoading = CountryFactory.retriveCountryList().query(vm.pagingInfo).$promise.then((country) => {
        if (country && country.data) {
          setDataAfterGetAPICall(country, false);
        }
    ...
    vm.getDataDown = () => {
      ...
      vm.cgBusyLoading = CountryFactory.retriveCountryList().query(vm.pagingInfo).$promise.then((country) => {
        if (country && country.data) {
          setDataAfterGetAPICall(country, true);
        }
      }).catch((error) => BaseService.getErrorLog(error));
    };
    ...
    // delete user
    vm.deleteRecord = (country) => {
      let selectedIDs = [];
      if (country) {
        selectedIDs.push(country.countryID);
      } else {
        vm.selectedRows = vm.selectedRowsList;
        if (vm.selectedRows.length > 0) {
          selectedIDs = vm.selectedRows.map((countryItem) => countryItem.countryID);
        }
      }
      if (selectedIDs) {
        const messageContent = angular.copy(CORE.MESSAGE_CONSTANT.DYNAMIC_MESSAGE.GLOBAL.DELETE_CONFIRM_MESSAGE);
        messageContent.message = stringFormat(messageContent.message, 'Country', selectedIDs.length);
        const obj = {
        ...
        DialogFactory.messageConfirmDialog(obj).then((yes) => {
          if (yes) {
            vm.cgBusyLoading = CountryFactory.deleteCountry().query({ objIDs: objIDs }).$promise.then((res) => {
              if (res && res.data) {
                if (res.data.length > 0 || res.data.transactionDetails) {
                  const data = {
                    TotalCount: res.data.transactionDetails[0].TotalCount,
                    pageName: CORE.PageName.country
                  };
                  BaseService.deleteAlertMessageWithHistory(data, (ev) => {
                    ...
                    return CountryFactory.deleteCountry().query({
                      objIDs: IDs
                    }).$promise.then((res) => {
                      let data = {};
                      data = res.data;
                      data.pageTitle = country ? country.countryName : null;
                      data.PageName = CORE.PageName.country;
        ...
        messageContent.message = stringFormat(messageContent.message, 'Country');
        const alertModel = {
          messageContent: messageContent
        };
    ...
    vm.addEditRecord = (data, ev) => {
      DialogFactory.dialogService(
        USER.ADMIN_COUNTRY_ADD_UPDATE_MODAL_CONTROLLER,
        USER.ADMIN_COUNTRY_ADD_UPDATE_MODAL_VIEW,
        ev,
        data).then(() => {
          BaseService.reloadUIGrid(vm.gridOptions, vm.pagingInfo, initPageInfo, vm.loadData);
    ...
    function cellEdit() {
      ...
      vm.gridOptions.gridApi.edit.on.afterCellEdit($scope, (rowEntity, colDef, newvalue, oldvalue) => {
        var obj = _.find(vm.sourceData, (item) => item.countryID === rowEntity.countryID);
        ...
        if (colDef.field === 'displayOrder' && newvalue !== oldvalue) {
          ...
          const countryModel = {
            ...
            countryID: rowEntity.countryID
          };
            vm.cgBusyLoading = CountryFactory.updateCountryDisplayOrder().save(countryModel).$promise.then((res) => {
              if (res) {
