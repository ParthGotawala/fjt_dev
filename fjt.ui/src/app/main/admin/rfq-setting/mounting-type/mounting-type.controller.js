(function () {
  ...
  angular
    ...
    .controller('MountingTypeController', MountingTypeController);

  /** @ngInject */
  function MountingTypeController(USER, $scope, CORE, RFQSettingFactory, BaseService, $timeout, DialogFactory, $mdDialog) {  // eslint-disable-line func-names
    const vm = this;
    ...
    vm.mountingAliasDetail = [];
    vm.MountingAliasCriteria = [];
    vm.isEditIntigrate = false;
    vm.loginUser = BaseService.loginUser;
    const isEnablePagination = vm.loginUser.uiGridPreference === CORE.UIGrid.UI_GRID_PAGING_PREFERENCE_TYPE.Pagination ? true : false;
    const CurrentPageName = CORE.PAGENAME_CONSTANT[53].PageName;
    ...
    },
    {
      ...
      field: 'numberOfPrintForUMID',
      displayName: 'Number of Print For UMID',
      cellTemplate: '<div class="ui-grid-cell-contents text-left">{{COL_FIELD}}</div>',
      width: '150',
      maxWidth: '200'
    },
    {
      field: 'limitedShelfLife',
      displayName: 'Has Limited Shelf Life',
      width: 120,
      cellTemplate: '<div class="ui-grid-cell-contents" style="text-align:center">'
        + '<span class="label-box" \
                            ng-class="{\'label-success\':row.entity.hasLimitedShelfLife == 1,\
                            \'label-warning\':row.entity.hasLimitedShelfLife == 0}"> \
                                {{COL_FIELD}}'
        + '</span>'
        + '</div>',
    ...
    {
      field: 'CountTypeEach',
      displayName: 'Count Type Each',
      width: 140,
      cellTemplate: '<div class="ui-grid-cell-contents" style="text-align:center">'
        + '<span class="label-box" \
                            ng-class="{\'label-success\':row.entity.isCountTypeEach == 1,\
                            \'label-warning\':row.entity.isCountTypeEach == 0}"> \
                                {{COL_FIELD}}'
        + '</span>'
        + '</div>',
      filterHeaderTemplate: '<div class="ui-grid-filter-container" ng-repeat="colFilter in col.filters"><div my-custom-dropdown></div></div>',
      ...
      width: '400',
      displayName: 'Alias',
      cellTemplate: '<div style="overflow: hidden" class="ui-grid-cell-contents" ng-repeat="item in row.entity.mountingAlias">\
    ...
    vm.sourceHeader.unshift(
      {
        ...
        cellTemplate: '<div class ="ui-grid-cell-contents cm-all-mfg-grid-checkbox" style="overflow:initial"><md-checkbox  ng-disabled="row.entity.isDisabledDelete"  ng-model="row.entity.isRecordSelectedForRemove" \
                            ng-change="grid.appScope.$parent.vm.setMountingTypeRemove(row.entity)"></md-checkbox></div>',
        enableFiltering: false,
        enableSorting: false,
        exporterSuppressExport: true,
        pinnedLeft: false,
    ...
    function setDataAfterGetAPICall(mountingType, isGetDataDown) {
      if (mountingType && mountingType.data.PartCategory) {
        bindData(mountingType.data.PartCategory);
        if (!isGetDataDown) {
          vm.sourceData = mountingType.data.PartCategory;
          vm.currentdata = vm.sourceData.length;
        }
        else if (mountingType.data.PartCategory.length > 0) {
          vm.sourceData = vm.gridOptions.data = vm.gridOptions.data.concat(mountingType.data.PartCategory);
          vm.currentdata = vm.gridOptions.currentItem = vm.gridOptions.data.length;
        }
        // must set after new data comes
        vm.totalSourceDataCount = mountingType.data.Count;
        ...
        if (vm.totalSourceDataCount === 0) {
          if (vm.pagingInfo.SearchColumns.length > 0 || !_.isEmpty(vm.SearchMode) || vm.MountingAliasCriteria.length > 0) {
    ...
    vm.loadData = () => {
      if (vm.isCheckAR) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[0].ID);
      }
      if (vm.isCheckAV) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[1].ID);
      }
      if (vm.isCheckDK) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[2].ID);
      }
      if (vm.isCheckMO) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[3].ID);
      }
      if (vm.isCheckNW) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[4].ID);
      }
      if (vm.isCheckTTI) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[5].ID);
      }
      if (vm.isCheckHEILIND) {
        vm.MountingAliasCriteria.push(vm.partSourceDetailsList[8].ID);
      }
      vm.pagingInfo.SourceDetail = vm.MountingAliasCriteria ? vm.MountingAliasCriteria.join(',') : vm.MountingAliasCriteria;
      vm.Apply = false;
      BaseService.setPageSizeOfGrid(vm.pagingInfo, vm.gridOptions);
      vm.cgBusyLoading = RFQSettingFactory.retriveMountingTypeList().query(vm.pagingInfo).$promise.then((mountingType) => {
        if (mountingType.status === CORE.ApiResponseTypeStatus.SUCCESS) {
          vm.pagingInfo.SourceDetail = null;
          setDataAfterGetAPICall(mountingType, false);
        }
        vm.MountingAliasCriteria = [];
    ...
    vm.getDataDown = () => {
      ...
      vm.cgBusyLoading = RFQSettingFactory.retriveMountingTypeList().query(vm.pagingInfo).$promise.then((mountingType) => {
        if (mountingType) {
          setDataAfterGetAPICall(mountingType, true);
        }
      }).catch((error) => BaseService.getErrorLog(error));
    };
    //bind data
    const bindData = (mountingDetail) => {
      _.each(mountingDetail, (item) => {
        const aliasDetails = [];
        const aliasList = [];
        const split = item.aliaslist ? item.aliaslist.split('##') : null;
        _.each(split, (aliasitem) => {
          vm.mountingAliasDetail = aliasitem ? aliasitem.split('@@@') : null;
          const aliasName = vm.mountingAliasDetail[0] ? vm.mountingAliasDetail[0] : null;
          const sourceDetails = vm.mountingAliasDetail[1] ? vm.mountingAliasDetail[1].split('#$#') : null;
          aliasDetails.push({
            aliasName: aliasName,
            sourceDetails: sourceDetails
          });
          aliasList.push(aliasName);
          item.mountingAlias = aliasDetails ? aliasDetails : null;
    ...
    function cellEdit() {
      ...
      vm.gridOptions.gridApi.edit.on.afterCellEdit($scope, (rowEntity, colDef, newvalue, oldvalue) => {
        ...
        if (colDef.field === 'displayOrder' && newvalue !== oldvalue) {
          ...
          const mountingTypeModel = {
            displayOrder: newvalue,
            id: rowEntity.id
          };
          vm.cgBusyLoading = RFQSettingFactory.updateMountingTypeDisplayOrder().save(mountingTypeModel).$promise.then((res) => {
            if (res) {
    ...
    vm.deleteRecord = (partcategory) => {
      ...
      if (selectedIDs) {
        ...
        DialogFactory.messageConfirmDialog(obj).then((resposne) => {
          if (resposne) {
            vm.cgBusyLoading = RFQSettingFactory.deleteMountingType().query({ objIDs: objIDs }).$promise.then((data) => {
              if (data && data.data && (data.data.length > 0 || data.data.transactionDetails)) {
                ...
                BaseService.deleteAlertMessageWithHistory(details, (ev) => {
                  ...
                  return RFQSettingFactory.deleteMountingType().query({
                    ...
                    data.PageName = CORE.PageName.mounting_type;
    ...
    //open popup for add edit new mounting type
    ...
    //refresh mounting type
    vm.refreshMountingType = () => {
      vm.loadData();
    };
