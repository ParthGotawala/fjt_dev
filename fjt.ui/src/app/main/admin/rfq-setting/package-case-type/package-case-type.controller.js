(function () {
  ...
  angular
    ...
    .controller('PackageCaseTypeController', PackageCaseTypeController);

  /** @ngInject */
  function PackageCaseTypeController(USER, $scope, CORE, RFQSettingFactory, BaseService, $timeout, DialogFactory, $mdDialog) {
    const vm = this;
    ...
    vm.CountTypeEachOptionsGridHeaderDropdown = CORE.CountTypeEachGridHeaderDropdown;
    ...
    vm.packageCaseAliasDetail = [];
    vm.PackageCaseAliasCriteria = [];
    ...
    vm.sourceHeader = [{
      ...
      field: 'XrayRequired',
      displayName: 'X-ray Required',
      width: 140,
      cellTemplate: '<div class="ui-grid-cell-contents" style="text-align:center">'
        + '<span class="label-box" \
                            ng-class="{\'label-success\':row.entity.isXrayRequired == 1,\
                            \'label-warning\':row.entity.isXrayRequired == 0}"> \
                                {{COL_FIELD}}'
        + '</span>'
        + '</div>',
      ...
      filter: {
        term: null,
        options: vm.CountTypeEachOptionsGridHeaderDropdown
      },
    ...
    //},
    {
      ...
      width: '330',
      displayName: 'Alias',
      cellTemplate: '<div style="overflow: hidden" class="ui-grid-cell-contents" ng-repeat="item in row.entity.packageCaseAlias">\
    ...
    vm.sourceHeader.unshift(
      {
        ...
        cellTemplate: '<div class ="ui-grid-cell-contents cm-all-mfg-grid-checkbox" style="overflow:initial"><md-checkbox  ng-disabled="row.entity.isDisabledDelete"  ng-model="row.entity.isRecordSelectedForRemove" \
                            ng-change="grid.appScope.$parent.vm.setPackageCaseTypeRemove(row.entity)"></md-checkbox></div>',
        enableFiltering: false,
        enableSorting: false,
        exporterSuppressExport: true,
        pinnedLeft: false,
    ...
    vm.gridOptions = {
      ...
      exporterCsvFilename: 'Package Case Type.csv',
      CurrentPage: CORE.PAGENAME_CONSTANT[61].PageName
    };

    function setDataAfterGetAPICall(packageCaseType, isGetDataDown) {
      if (packageCaseType && packageCaseType.data.PartCategory) {
        bindData(packageCaseType.data.PartCategory);
        if (!isGetDataDown) {
          vm.sourceData = packageCaseType.data.PartCategory;
          vm.currentdata = vm.sourceData.length;
        }
        else if (packageCaseType.data.PartCategory.length > 0) {
          vm.sourceData = vm.gridOptions.data = vm.gridOptions.data.concat(packageCaseType.data.PartCategory);
          vm.currentdata = vm.gridOptions.currentItem = vm.gridOptions.data.length;
        }
        // must set after new data comes
        vm.totalSourceDataCount = packageCaseType.data.Count;
        ...
        if (vm.totalSourceDataCount === 0) {
          if (vm.pagingInfo.SearchColumns.length > 0 || !_.isEmpty(vm.SearchMode) || vm.PackageCaseAliasCriteria.length > 0) {
    ...
    vm.loadData = () => {
      if (vm.isCheckAR) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[0].ID);
      }
      if (vm.isCheckAV) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[1].ID);
      }
      if (vm.isCheckDK) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[2].ID);
      }
      if (vm.isCheckMO) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[3].ID);
      }
      if (vm.isCheckNW) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[4].ID);
      }
      if (vm.isCheckTTI) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[5].ID);
      }
      if (vm.isCheckHEILIND) {
        vm.PackageCaseAliasCriteria.push(vm.partSourceDetailsList[8].ID);
      }
      vm.pagingInfo.SourceDetail = vm.PackageCaseAliasCriteria !== null ? vm.PackageCaseAliasCriteria.join(',') : vm.PackageCaseAliasCriteria;
      vm.Apply = false;
      BaseService.setPageSizeOfGrid(vm.pagingInfo, vm.gridOptions);
      vm.cgBusyLoading = RFQSettingFactory.retrivePackageCaseTypeList().query(vm.pagingInfo).$promise.then((packageCaseType) => {
        if (packageCaseType.status === CORE.ApiResponseTypeStatus.SUCCESS) {
          vm.pagingInfo.SourceDetail = null;
          setDataAfterGetAPICall(packageCaseType, false);
        }
        vm.PackageCaseAliasCriteria = [];
    ...
    vm.getDataDown = () => {
      ...
      vm.cgBusyLoading = RFQSettingFactory.retrivePackageCaseTypeList().query(vm.pagingInfo).$promise.then((packageCaseType) => {
        if (packageCaseType) {
          setDataAfterGetAPICall(packageCaseType, true);
        }
      }).catch((error) => BaseService.getErrorLog(error));
    };
    //bind data
    const bindData = (packageCaseDetail) => {
      _.each(packageCaseDetail, (item) => {
        const aliasDetails = [];
        const aliasList = [];
        const split = item.aliaslist ? item.aliaslist.split('##') : null;
        _.each(split, (aliasitem) => {
          vm.packageCaseAliasDetail = aliasitem ? aliasitem.split('@@@') : null;
          const aliasName = vm.packageCaseAliasDetail[0] ? vm.packageCaseAliasDetail[0] : null;
          const sourceDetails = vm.packageCaseAliasDetail[1] ? vm.packageCaseAliasDetail[1].split('#$#') : null;
          aliasDetails.push({
            aliasName: aliasName,
            sourceDetails: sourceDetails
          });
          aliasList.push(aliasName);
          item.packageCaseAlias = aliasDetails ? aliasDetails : null;
    ...
    vm.deleteRecord = (partcategory) => {
      ...
      if (selectedIDs) {
        ...
        messageContent.message = stringFormat(messageContent.message, 'Package/Case(Shape) Type', selectedIDs.length);
        const obj = {
        ...
        DialogFactory.messageConfirmDialog(obj).then((resposne) => {
          if (resposne) {
            vm.cgBusyLoading = RFQSettingFactory.deletePackageCaseType().query({ objIDs: objIDs }).$promise.then((data) => {
              if (data && data.data && (data.data.length > 0 || data.data.transactionDetails)) {
                const dataObj = {
                  TotalCount: data.data.transactionDetails[0].TotalCount,
                  pageName: CORE.PageName.package_case_type
                };
                BaseService.deleteAlertMessageWithHistory(dataObj, (ev) => {
                  ...
                  return RFQSettingFactory.deletePackageCaseType().query({
                    objIDs: IDs
                  }).$promise.then((res) => {
                    let data = {};
                    data = res.data;
                    data.pageTitle = partcategory ? partcategory.name : null;
                    data.PageName = CORE.PageName.package_case_type;
        ...
        messageContent.message = stringFormat(messageContent.message, 'Package/Case(Shape) Type');
        const alertModel = {
          messageContent: messageContent,
          multiple: true
        };
    ...
    //open popup for add edit new package case type
    vm.addEditRecord = (data, ev) => {
      ...
      DialogFactory.dialogService(
        USER.ADMIN_MANAGE_PACKAGE_CASE_TYPE_MODAL_CONTROLLER,
        USER.ADMIN_MANAGE_PACKAGE_CASE_TYPE_MODAL_VIEW,
        ev,
        objdata).then((result) => {
          if (result) {
            BaseService.reloadUIGrid(vm.gridOptions, vm.pagingInfo, initPageInfo, vm.loadData);
    ...
    vm.showAlias = (row, ev) => {
      const model = {
        refTableName: CORE.TABLE_NAME.RFQ_PACKAGECASETYPE,
        refId: row.id,
        isEdit: true,
    ...
    //refresh package case type
    vm.refreshPackageCaseType = () => {
      vm.loadData();
    };
    ...
    vm.applyAll = (applyAll) => {
      if (applyAll) {
        _.map(vm.sourceData, selectMounting);
      } else {
        _.map(vm.sourceData, unselectMounting);
      }
    };
    const selectMounting = (row) => {
    ...
    const unselectMounting = (row) => {
      row.isRecordSelectedForRemove = false;
      vm.gridOptions.clearSelectedRows();
    };
    vm.setPackageCaseTypeRemove = (row) => {
