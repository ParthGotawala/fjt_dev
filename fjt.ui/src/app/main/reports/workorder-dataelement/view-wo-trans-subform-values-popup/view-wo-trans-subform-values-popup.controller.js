(function () {
  ...
  angular
    ...
    .controller('ViewWOTransSubFormValuesPopupController', ViewWOTransSubFormValuesPopupController);

  /** @ngInject */
  function ViewWOTransSubFormValuesPopupController($scope, $timeout, $state, $mdDialog, CORE, $filter,
    DialogFactory, data, BaseService, WorkorderDataelementFactory, REPORTS, WorkorderDataElementTransValueFactory) {
    const vm = this;
    ...
    vm.gridOptions = {
      ...
      exporterCsvFilename: 'Work Order Operation SubForm Transaction Details.csv',
    ...
    vm.loadData = () => {
      ...
      vm.cgBusyLoading = WorkorderDataelementFactory.getWoTransactionDataElementValuesList().query(vm.pagingInfo).$promise.then((elements) => {
        vm.sourceData = elements.data.woTransDEValuesList;
    ...
    vm.getDataDown = () => {
      ...
      vm.cgBusyLoading = WorkorderDataelementFactory.getWoTransactionDataElementValuesList().query(vm.pagingInfo).$promise.then((elements) => {
        ...
        //     }
        // });
        vm.sourceData = vm.gridOptions.data = vm.sourceData.concat(elements.data.woTransDEValuesList);
