(function () {
    ...
    angular
        ...
        .controller('ExcessMaterialController', ExcessMaterialController);

    /** @ngInject */
    function ExcessMaterialController($filter, $state, $scope, $timeout, $stateParams, $mdSidenav, RFQTRANSACTION, BOMFactory, PartCostingFactory,
        ...
        vm.SelectedTabName = PRICING.REVIEW_PRICING_TABS.ExcessMaterialExposure.Name;
        ...
        vm.gridOptions = {
            ...
            exporterCsvFilename: 'Excess Material.csv',
        ...
        vm.loadData = () => {
            vm.sourceData = [];
            vm.cgBusyLoading = PartCostingFactory.retrieveExcessMaterialLineItems(vm.pagingInfo).query().$promise.then((consolidatemfg) => {
                        ...
                        _.each(vm.partcostingKeys, (keys) => {
                            if (BaseService.checkForVisibleColumnInGrid(PartCosting, keys, vm.sourceHeader)
                                ...
                                var obj = {
                                    ...
                                    footerCellTemplate: '<div class="ui-grid-cell-contents" >{{grid.appScope.$parent.vm.formatToSixDigit(col)}}</div>',
                                    //enableGrouping: true,
                                }
        ...
        // Conver tot Six digit decimal number
        vm.formatToSixDigit = (col) => {
            let value = BaseService.formatToSixDigit(col);
            return value ? "Excess Total: $ " + value : "";
        }
