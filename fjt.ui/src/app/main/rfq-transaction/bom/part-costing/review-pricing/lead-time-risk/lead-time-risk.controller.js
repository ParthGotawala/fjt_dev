(function () {
    ...
    angular
        ...
        .controller('LeadTimeRiskController', LeadTimeRiskController);

    /** @ngInject */
    function LeadTimeRiskController($filter, $state, $scope, $timeout, $stateParams, $mdSidenav, RFQTRANSACTION, BOMFactory, PartCostingFactory,
        ...
        vm.SelectedTabName = PRICING.REVIEW_PRICING_TABS.LeadTimeRisk.Name;
        ...
        vm.gridOptions = {
            ...
            exporterCsvFilename: 'Lead Time Risk.csv',
        ...
        vm.loadData = () => {
            vm.sourceData = [];
            vm.cgBusyLoading = PartCostingFactory.retrieveLeadTimeRiskLineItems(vm.pagingInfo).query().$promise.then((consolidatemfg) => {
                    ...
                    if (vm.sourceData.length > 0) {
                        ...
                        _.each(vm.partcostingKeys, (keys) => {
                            if (BaseService.checkForVisibleColumnInGrid(PartCosting, keys, vm.sourceHeader)
                                ...
                                && keys != PartCosting.RequestQty
                                && keys != PartCosting.ApiLead
                                && keys != PartCosting.Min
                                && keys != PartCosting.Mult
                                && keys != PartCosting.CurrentStock) {
                                var obj = {
                                    field: keys,
                                    displayName: keys + ' (LeadTimeNotes)',
