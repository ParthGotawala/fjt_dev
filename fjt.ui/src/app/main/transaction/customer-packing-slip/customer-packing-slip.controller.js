(function () {
  ...
  angular.module('app.transaction.customerpacking')
    .controller('CustomerPackingSlipController', CustomerPackingSlipController);

  /** @ngInject */
  function CustomerPackingSlipController($state, TRANSACTION, BaseService, MyProfileFactory, CORE, $filter) {
    const vm = this;
    ...
    vm.tabList = [
      {
        ...
        title: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_SUMM_LABEL,
        state: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_SUMM_STATE,
        src: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_SUMM_STATE,
      ...
      },
      {
        ...
        title: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_DETL_LABEL,
        // state: TRANSACTION.TRANSACTION_CUSTOMER_INVOICE_STATE,
        state: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_DETL_STATE,
        src: TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_DETL_STATE,
    ...
    const setActiveTab = () => {
      if (vm.loginUser) {
        ...
        if ($state.params.psNumber) {
          ...
          $state.go(TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_SUMM_STATE);
          //}
          ...
          //if ($state.current.name !== TRANSACTION.TRANSACTION_CUSTOMER_INVOICE_STATE) {
          // above condition added to resolve transition suspended console issue
          $state.go(TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_DETL_STATE);
          //}
        } else {
          vm.activeTab = TRANSACTION.CustomerPackingSlipListTabIDs.SummaryList;
          $state.go(TRANSACTION.TRANSACTION_CUSTOMER_PACKINGSLIP_LIST_SUMM_STATE);
      ...
      vm.cgBusyLoading = MyProfileFactory.saveUserConfiguration().query(objTab).$promise.then((resp) => {
        if (resp && resp.status === CORE.ApiResponseTypeStatus.SUCCESS) {
          ...
          if (tractActivityLog && Array.isArray(tractActivityLog)) {
            const obj = { time: new Date().toString().replace(/T/, ':').replace(/\.\w*/, '') , message: 'Set Loginuser: packing slip.' };
