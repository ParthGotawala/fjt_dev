(function () {
  ...
  angular
    .module('app.core')
    .controller('ScanUMIDComponentPopupController', ScanUMIDComponentPopupController);

  /** @ngInject */
  function ScanUMIDComponentPopupController($scope, $mdDialog, $filter, data, BaseService, TRAVELER, CORE, $timeout, USER,
    ...
    vm.data = data;
    vm.isShowLight = true;
    vm.isHideDelete = vm.data.isVerify ? true : false;
    ...
    vm.stockType = 'ALL';
    vm.title = stringFormat('{0} Material', vm.data.isVerify ? 'Verify' : 'Scan');
    vm.InvalidRefDesMessage = stringFormat(CORE.MESSAGE_CONSTANT.INVALID_DYNAMIC, vm.LabelConstant.BOM.REF_DES);
    let commonItems = []; // used to  bind source header common column dynamically
    vm.goToComponentBOM = () => {
    ...
    */
    ...
    const sourceHeaderCommonItem = () => {
      ...
      commonItems.push(
        ...
        {
          field: 'refDesig',
          displayName: vm.IsShowEditMaterials ? vm.LabelConstant.BOM.REF_DES + ' (Modify below as required)' : vm.LabelConstant.BOM.REF_DES,
          width: 120,
        ...
        {
          field: 'assyQty',
          displayName: vm.IsShowEditMaterials ? 'Assy Qty (Modify below as required)' : 'Assy Qty',
          width: 120,
    ...
    vm.resetAll = () => {
      ...
      if (vm.ScanUMIDDetails) {
        vm.ScanUMIDDetails.$setPristine();
        vm.ScanUMIDDetails.$setUntouched();
      }
      setFocusByName('umid');
    ...
    // close popup
    vm.cancel = () => {
      const isdirty = vm.checkFormDirty(vm.ScanUMIDDetails);
      if (isdirty) {
        const data = {
          form: vm.ScanUMIDDetails
        };
    ...
    vm.validateUMIDDetails = (e, authenticationApprovedDet) => {
      const umidObj = {
        ...
        isPlacementTracking: vm.umidScan.isPlacementTracking ? vm.umidScan.isPlacementTracking : false,
      ...
      vm.cgBusyLoading = WorkorderTransactionUMIDFactory.validateScanUMIDOnly().query({ umidObj: umidObj }).$promise.then((res) => {
        ...
        if (res.data && res.data.umidDetails && res.data.umidDetails.length > 0) {
          ...
          vm.umidDetails.pkgQty = (vm.umidDetails.umidUOMClassID === CORE.MEASUREMENT_TYPES.COUNT.ID ? vm.umidDetails.pkgQty : $filter('unit')(vm.umidDetails.pkgQty));
        }
    ...
    vm.setLineItemDetails = (item) => {
      ...
      vm.umidScan.refdes = item.refDesig;
      keyEvent.keyCode = 13;
                ...
                if (res.data.length > 0 || res.data.transactionDetails) {
                  ...
                  BaseService.deleteAlertMessageWithHistory(data, (ev) => {
                    ...
                    return WorkorderTransactionUMIDFactory.deleteWorkorderTransUMIDDetails().query({ objIDs: objIDs }).$promise.then((res) => {
                      let data = {};
                      data = res.data;
                      data.pageTitle = workOrderTransUMID ? workOrderTransUMID.umid : null;
                      data.PageName = CORE.PageName.workorder_transaction_umid_details;
                      data.selectedIDs = stringFormat('{0}{1}', selectedIDs.length, ' Selected');
                      data.controllerName = TRAVELER.SCAN_UMID_VIEW_HISTORY_MODAL_CONTROLLER;
                      data.viewTemplateUrl = TRAVELER.SCAN_UMID_VIEW_HISTORY_MODAL_VIEW;
                      const popUpHeaderData = {
                        equipmentName: null,
                        wo: vm.data.woNumber,
                        version: vm.data.woVersion,
                      ...
                      data.redirectData = { isScanUMIDOnly: true, woID: vm.data.woID, isScanFeederFirstAndUMIDFirstAndChangeReel: false, woOPID: vm.data.woOPID ? vm.data.woOPID : null, headerData: popUpHeaderData };
                      if (res.data) {
                        DialogFactory.dialogService(
    ...
    angular.element(() => {
      //check load
      BaseService.currentPagePopupForm.push(vm.ScanUMIDDetails);
    });
    ...
    vm.saveScanUMIDDetails = (e) => {
      if (vm.ScanUMIDDetails.$invalid) {
        BaseService.focusRequiredField(vm.ScanUMIDDetails);
        return false;
      }
    ...
    /** Validate Ref. Desg */
    vm.validateRefDesg = () => {
      let selectedBomLineList = [];
      vm.umidScan.errorText = '';
      if (vm.umidScan.refdes) {
        let refDesg_list = getDesignatorFromLineItem(vm.umidDetails ? vm.umidDetails.refDesig : [],vm.data.bomOddlyRefDesList);
        if (refDesg_list && refDesg_list.length === 0) {
          selectedBomLineList = _.map(vm.selectedbomLineItemList, (det) => det.refDesig);
          refDesg_list = getDesignatorFromLineItem(selectedBomLineList ? selectedBomLineList.join(',') : [], vm.data.bomOddlyRefDesList);
        }
        ...
        if (notInList.length > 0) {
          $scope.$applyAsync(() => {
            vm.ScanUMIDDetails.refdes.$setValidity('invalidrefdesg', false);
          });
          return false;
        }
        if (!vm.umidScan.rfqLineItemsID) {
          vm.umidScan.rfqLineItemsID = vm.selectedbomLineItemList && vm.selectedbomLineItemList.length > 0 ? _.find(vm.selectedbomLineItemList, (det) => det.refDesig === vm.umidScan.refdes).rfqLineItemsID : null;
        }
        vm.umidScan.refdes = entered_list && entered_list.length > 0 ? entered_list.join(',') : vm.umidScan.refdes;
        vm.ScanUMIDDetails.refdes.$setValidity('invalidrefdesg', true);
      }
    };
    ...
    const validateScanMaterialDetails = (colDef, rowEntity, isEdit) => {
      ...
      if (isEdit) {
        ...
        let refDesg_list = getDesignatorFromLineItem(rowEntity.validRefDesig, vm.data.bomOddlyRefDesList);
        if (refDesg_list && refDesg_list.length === 0) {
          const selectedBomLineList = _.map(vm.selectedbomLineItemList, (det) => det.refDesig).join(',');
          refDesg_list = getDesignatorFromLineItem(selectedBomLineList ? selectedBomLineList : [], vm.data.bomOddlyRefDesList);
        }
    ...
    if (!vm.data.isDisableScan && vm.data.woSubStatus === CORE.WOSTATUS.COMPLETED_WITH_MISSING_PARTS) {
